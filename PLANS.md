...
| ~~ML-DSA: Sign Failure Path Zeroization~~ | ~~MEDIUM~~ | ~~Security~~ | Done — added explicit `.zeroize()` calls for `rho_prime`, `s1`, `s2`, `t0`, `s1_hat`, `s2_hat`, `t0_hat` on `MAX_ATTEMPTS` failure path |
