...
| ML-DSA: Drop-based Zeroization | MEDIUM | Security | `ml_dsa_sign` uses manual `.zeroize()` calls on each rejection/success path. This is fragile — future control-flow changes can easily introduce gaps. Migrate loop-scoped sensitive intermediates to `Zeroizing<T>` wrappers or a loop-local struct with `ZeroizeOnDrop` for automatic cleanup on all paths (including panics). |
