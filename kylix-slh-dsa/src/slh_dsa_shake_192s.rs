...
/// SLH-DSA-SHAKE-192s signing key.
...
/// SLH-DSA-SHAKE-192s verification key.
...
/// SLH-DSA-SHAKE-192s algorithm marker type.
pub struct SlhDsaShake192s;

impl Signer for SlhDsaShake192s {
    ...
    fn keygen(rng: &mut impl CryptoRng) -> Result<(Self::SigningKey, Self::VerificationKey)> {
        let (sk, pk) = slh_keygen::<Shake192Hash, N, WOTS_LEN, H_PRIME, D>(rng);
        Ok((SigningKey(sk), VerificationKey(pk)))
    }

    fn sign(sk: &Self::SigningKey, message: &[u8]) -> Result<Self::Signature> {
        let sig = slh_sign::<Shake192Hash, N, WOTS_LEN, WOTS_LEN1, H_PRIME, D, K, A, MD_BYTES>(
            &sk.0, message, None,
        );
        Ok(Signature(sig))
    }
...
mod tests {
    ...
    fn test_keygen_sign_verify() {
        ...
        let (sk, pk) = SlhDsaShake192s::keygen(&mut rng).unwrap();

        let message = b"Hello, SLH-DSA-SHAKE-192s!";
        let signature = SlhDsaShake192s::sign(&sk, message).unwrap();

        assert!(SlhDsaShake192s::verify(&pk, message, &signature).is_ok());
    }

    #[test]
    fn test_key_sizes() {
        assert_eq!(SlhDsaShake192s::SIGNING_KEY_SIZE, 96);
        assert_eq!(SlhDsaShake192s::VERIFICATION_KEY_SIZE, 48);
        assert_eq!(SlhDsaShake192s::SIGNATURE_SIZE, 16224);
    }
}
