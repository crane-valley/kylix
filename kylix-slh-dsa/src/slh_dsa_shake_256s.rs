...
/// SLH-DSA-SHAKE-256s signing key.
...
/// SLH-DSA-SHAKE-256s verification key.
...
/// SLH-DSA-SHAKE-256s algorithm marker type.
pub struct SlhDsaShake256s;

impl Signer for SlhDsaShake256s {
...
mod tests {
    ...
    fn test_keygen_sign_verify() {
        ...
        let (sk, pk) = SlhDsaShake256s::keygen(&mut rng).unwrap();

        let message = b"Hello, SLH-DSA-SHAKE-256s!";
        let signature = SlhDsaShake256s::sign(&sk, message).unwrap();

        assert!(SlhDsaShake256s::verify(&pk, message, &signature).is_ok());
    }

    #[test]
    fn test_key_sizes() {
        assert_eq!(SlhDsaShake256s::SIGNING_KEY_SIZE, 128);
        assert_eq!(SlhDsaShake256s::VERIFICATION_KEY_SIZE, 64);
        assert_eq!(SlhDsaShake256s::SIGNATURE_SIZE, 29792);
    }
}
