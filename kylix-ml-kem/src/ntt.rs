...
mod tests {
    ...
    fn test_ntt_inv_ntt_roundtrip() {
        ...
        for i in 0..256 {
            ...
            assert_eq!(reduced, orig, "NTT roundtrip failed at index {}: got {}, expected {}", i, reduced, orig);
        }
    }
    ...
    fn test_ntt_zero_polynomial() {
        ...
        for i in 0..256 {
            assert_eq!(poly.coeffs[i], 0, "NTT of zero polynomial should be zero at index {}", i);
        }
    }
    ...
    fn test_zetas_in_valid_range() {
        for (i, &z) in ZETAS.iter().enumerate() {
            assert!(z >= 0 && z < 3329, "ZETAS[{}] = {} out of range [0, 3329)", i, z);
        }
    }
